...
struct ReprojectionError
{
  ...
  // Struct for computing reprojection error used in Structure from Motion (SfM).
  ReprojectionError(double observed_x_val, double observed_y_val)
    : observed_x(observed_x_val), observed_y(observed_y_val) {}

  template <typename T>
  bool operator()(const T* const camera_params, const T* const point_coords, T* residuals) const {
    // Rotating the point using the camera's rotation parameters.
    T p[3];
    ceres::AngleAxisRotatePoint(camera_params, point_coords, p);

    // Translating the point using the camera's translation parameters.
    p[0] += camera_params[3];
    p[1] += camera_params[4];
    p[2] += camera_params[5];

    // Projecting the point onto the image plane using a normalized camera model.
    T xp = p[0] / p[2];
    T yp = p[1] / p[2];

    // Computing the residuals as the difference between observed and projected coordinates.
  ...
  static ceres::CostFunction* Create(const double observed_x_val,
                                     const double observed_y_val) {
    // Factory function to create an instance of AutoDiffCostFunction.
    return (new ceres::AutoDiffCostFunction<ReprojectionError, 2, 6, 3>(
               new ReprojectionError(observed_x_val, observed_y_val)));
  }

  // Observed 2D coordinates in the image.
  double observed_x;
  double observed_y;
...
bool BasicSfM::incrementalReconstruction( int seed_pair_idx0, int seed_pair_idx1 )
{
  ...
  //retrieve the essential and homograph matrices
  cv::Mat E = cv::findEssentialMat(points0, points1, intrinsics_matrix, cv::RANSAC, 0.999, 0.1, inlier_mask_E);
  cv::Mat H = cv::findHomography(points0, points1, cv::RANSAC, 0.1, inlier_mask_H);

  //for both models, find the number of corresponding inliers
  int essentialInliers = 0;
  int homographInliers = 0;

  for (int i = 0; i < inlier_mask_E.rows; i++) {
    if (inlier_mask_E.at<unsigned char>(i))
      essentialInliers++;
  }

  for (int i = 0; i < inlier_mask_H.rows; i++) {
    if (inlier_mask_H.at<unsigned char>(i))
      homographInliers++;
  }

  //and check that the inliers of E are more than the ones of H. Otherwise, stop the execution for this seed pair
  if (essentialInliers < homographInliers)
    return false;

  //store in init_r_mat and init_t_vec the recovered rotation and translation
  cv::recoverPose(E, points0, points1, intrinsics_matrix, init_r_mat, init_t_vec, inlier_mask_E);

  //in the translation vector just recovered, x provides the sideward motion, while z the forward one (wrt the camera).
  //Checking the absolute value of both allows to determine the main direction of motion
  ...
  for(int iter = 1; iter < num_cam_poses_ - 1; iter++ )
  {
      ...
      if( cam_pose_optim_iter_[cam_idx] > 0 )
      {
        for( auto const& co_iter : cam_observation_[cam_idx] )
        {
              ...
              cam_observation_[new_cam_pose_idx].find(pt_idx ) != cam_observation_[new_cam_pose_idx].end() )
          {
            ...
            //build the projection transformation matrices for both cameras
            cv::Mat_<double> camera_R_vec(3,1);
            cv::Mat_<double> camera_t_vec(3,1);
            
            for(int k = 0; k < camera_R_vec.rows; k++) {
                camera_R_vec.at<double>(k) = cam0_data[k];
                camera_t_vec.at<double>(k) = cam0_data[k + 3];
            }
            
            cv::Mat_<double> R(3, 3);

            //recover the rotation matrix from the axis-angle rotation vector
            cv::Rodrigues(camera_R_vec, R);

            //build the projected transformation matrix
            R.copyTo(proj_mat0(cv::Rect(0, 0, 3, 3)));
            camera_t_vec.copyTo(proj_mat0(cv::Rect(3, 0, 1, 3)));

            //repeat the same process for the other camera

            for(int k = 0; k < camera_R_vec.rows; k++) {
                camera_R_vec.at<double>(k) = cam1_data[k];
                camera_t_vec.at<double>(k) = cam1_data[k + 3];
            }

            cv::Rodrigues(camera_R_vec, R);
            R.copyTo(proj_mat1(cv::Rect(0, 0, 3, 3)));
            camera_t_vec.copyTo(proj_mat1(cv::Rect(3, 0, 1, 3)));

            //store the 2D projection of the observed 3D point
            points0.emplace_back(observations_[cam_observation_[new_cam_pose_idx][co_iter.first] * 2], observations_[cam_observation_[new_cam_pose_idx][co_iter.first] * 2 + 1]);
            ...
            //cheirality constraint. Since it requires the point to be in front of the camera,
            //this means that its forward coordinate (corresponding to the z) must be positive
            if(hpoints4D.at<double>(2) / hpoints4D.at<double>(3) > 0.0) {
              n_new_pts++;
              pts_optim_iter_[pt_idx] = 1;
              double* pt = pointBlockPtr(pt_idx);
    ...
    //save the parameters before the bundle adjustment to check for divergence
    std::vector<double> parameters_prev = parameters_;
    ...
    float camera_threshold = 0.09;
    int diverged_poses = 0;

    //check the camera poses to see if the reconstruction has diverged
    for(int k = 0; k < num_cam_poses_; k++) {
      for(int p = 0; p < 6; p++) {
        //for each pose, check each component separately to see if at least one of them "diverged" after
        //the refinement. If so, consider the pose "diverged", increase the counter and move to the next
        //pose
        if(std::abs(parameters_prev[k * 6 + p]) < camera_threshold * std::abs(parameters_[k * 6 + p])) {
          diverged_poses++;
          break;
        }
      }
    ...
    if(diverged_poses > num_cam_poses_ / 2) {
      return false;
    }

    //retrieve all the points of the previous and current iteration
    std::vector<cv::Point3d> currPoints;
    std::vector<cv::Point3d> prevPoints;
    
    for(int k = num_cam_poses_ * 6; k < parameters_.size(); k+=3) {
      //this check is technically not needed since previous parameters were checked in the previous iteration,
      //so there shouldn't be any point with infinite coordinates
      if(!isinf(parameters_prev[k]) && !isinf(parameters_prev[k+1]) && !isinf(parameters_prev[k+2]))
        prevPoints.push_back(cv::Point3d(parameters_prev[k], parameters_prev[k+1], parameters_prev[k+2]));

      //for the current iteration store the non infinite points. If there is any, simply discard it
      if(!isinf(parameters_[k]) && !isinf(parameters_[k+1]) && !isinf(parameters_[k+2])) 
        currPoints.push_back(cv::Point3d(parameters_[k], parameters_[k+1], parameters_[k+2]));
    }    

    //BEFORE REFINEMENT DATA

    //define the matrix that will contain all 3D points
    cv::Mat data_pts_prev = cv::Mat(prevPoints.size(), 3, CV_64FC1);

    //store the 3D points as rows in the data matrix
    for (int k = 0; k < data_pts_prev.rows; k++)
    {
      data_pts_prev.at<double>(k, 0) = prevPoints[k].x;
      data_pts_prev.at<double>(k, 1) = prevPoints[k].y;
      data_pts_prev.at<double>(k, 2) = prevPoints[k].z;      
    }

    //perform the PCA analysis
    cv::PCA pca_analysis_prev(data_pts_prev, cv::Mat(), cv::PCA::DATA_AS_ROW);
    
    //and store the resulting eigenvalues and eigenvectors
    std::vector<cv::Point3d> eigen_vecs_prev(3);
    std::vector<double> eigen_val_prev(3);

    for (int k = 0; k < 3; k++)
    {
      eigen_vecs_prev[k] = cv::Point3d(pca_analysis_prev.eigenvectors.at<double>(k, 0), pca_analysis_prev.eigenvectors.at<double>(k, 1), pca_analysis_prev.eigenvectors.at<double>(k, 2));
      eigen_val_prev[k] = pca_analysis_prev.eigenvalues.at<double>(k);
    }

    //AFTER REFINEMENT DATA

    //repeat the same steps done before for the 3D points after the bundleAdjustment
    cv::Mat data_pts_curr = cv::Mat(currPoints.size(), 3, CV_64FC1);

    for (int k = 0; k < data_pts_curr.rows; k++)
    {
      data_pts_curr.at<double>(k, 0) = currPoints[k].x;
      data_pts_curr.at<double>(k, 1) = currPoints[k].y;
      data_pts_curr.at<double>(k, 2) = currPoints[k].z;      
    }

    cv::PCA pca_analysis_curr(data_pts_curr, cv::Mat(), cv::PCA::DATA_AS_ROW);

    std::vector<cv::Point3d> eigen_vecs_curr(3);
    std::vector<double> eigen_val_curr(3);

    for (int k = 0; k < 3; k++)
    {
      eigen_vecs_curr[k] = cv::Point3d(pca_analysis_curr.eigenvectors.at<double>(k, 0), pca_analysis_curr.eigenvectors.at<double>(k, 1), pca_analysis_curr.eigenvectors.at<double>(k, 2));
      eigen_val_curr[k] = pca_analysis_curr.eigenvalues.at<double>(k);
    }

    //set the threshold value to determine if the reconstruction has diverged or not
    //In this case, it does if in the current iteration the eigenvalues are more than double the value of the ones before the update
    double eigen_threshold = 0.5;
    
    for (int k = 0; k < 3; k++) {
      if(eigen_val_prev[k] < eigen_threshold * eigen_val_curr[k]) {
        return false;
      }
    }
  ...
  while( keep_optimize )
  {
    ...
    for( int i_obs = 0; i_obs < num_observations_; i_obs++ )
    {
      ...
      if( cam_pose_optim_iter_[cam_pose_index_[i_obs]] > 0 && pts_optim_iter_[point_index_[i_obs]] > 0 )
      {
        ...
        int c_index = cam_pose_index_[i_obs];
        int p_index = point_index_[i_obs];

        ceres::CostFunction *cost_function = ReprojectionError::Create(observations_[2 * i_obs], observations_[2 * i_obs + 1]);

        double *camera_pointer = parameters_.data() + (camera_block_size_ * c_index);
        double *point_pointer = parameters_.data() + (camera_block_size_ * num_cam_poses_ + point_block_size_ * p_index);

        problem.AddResidualBlock(cost_function, new ceres::CauchyLoss(2*max_reproj_err_), camera_pointer, point_pointer);
